...
Gnu documentation on =findutils= [[https://www.gnu.org/software/findutils/][here]].

#+begin_src emacs-lisp
  (defun my-clean-up-line-endings-command (dir pat)
    "Walk the directory tree DIR, running `dos2unix' on all files
  matching glob pattern PAT, a string."
    (string-join
     (list "find" dir "-name" (concat "\"" pat "\"") "-print0 | xargs -0 dos2unix")
     " "))
#+end_src
