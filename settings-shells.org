...
#+begin_src emacs-lisp
  ...
  conda environment activated."
    ...
    (cond
     ((my-remote-buffer-p)
      ;; Assumes remote host is running Linux
      (setq explicit-shell-file-name "/usr/bin/bash")
      (setq shell-file-name explicit-shell-file-name)
      (setq explicit-bash-args (list "-i" "-l")))
      ;; (call-interactively #'shell))
     ((eq system-type 'windows-nt)
      ;; Assumes MSYS2 is installed
      (unless (bound-and-true-p my-msys2-dir)
        (error "Site-local variable `my-msys2-dir' not set"))
      (let ((msys2-bash (concat my-msys2-dir "/usr/bin/bash.exe")))
        (setq explicit-shell-file-name msys2-bash)
        (setq shell-file-name msys2-bash)
        (setenv "SHELL" msys2-bash))
      (setq explicit-bash.exe-args (list "-i" "-l"))
      ;; https://www.gnu.org/software/emacs/manual/html_node/efaq-w32/Bash.html
      (setenv "PID" nil))
     (t
      ;; Assumes Unix-like OS
    ...
    (let ((default-directory (my-shell-dir)))
      (when (and
             (bound-and-true-p conda-env-autoactivate-mode)
...
There's another package, [[https://github.com/atomontage/xterm-color][xterm-color]], that purports to colorize shell
buffers better than [[https://www.emacswiki.org/emacs/AnsiColor][ansi-color]]. I don't use either one. See also
Moostropfen's [[help:ansi-color-map][ansi-color-map]] [[https://www.reddit.com/r/emacs/comments/mxf7v6/how_to_display_colors_in_mx_shell/][settings]].
#+begin_src emacs-lisp
...
Use the [[https://github.com/akermu/emacs-libvterm][vterm]] library for a better Emacs shell.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (defun my-vterm-start ()
    "Start a new vterm at the project root, with the current buffer's
  conda environment activated."
    (interactive)
    (let ((default-directory (my-shell-dir)))
      (when (and
...
#+begin_src emacs-lisp
  (defun my-vterm-close (&optional buffer)
    "Close the vterm window without killing its buffer or process."
    (interactive)
    (let ((buffer (or buffer (current-buffer))))
      (unless (eq
               (buffer-local-value 'major-mode buffer)
               'vterm-mode)
        (user-error "Buffer '%s' is not running vterm." buffer))
      (let ((window (get-buffer-window buffer)))
        ;; Borrowed from popper.el
        (when (window-valid-p window)
          (cond
...
#+begin_src emacs-lisp
  (use-package chatgpt-shell
    ...
    (shell-maker-transcript-default-path "~/working/tmp/"))
    ;; :config
