...
(defcustom git-gutter:git-program "git"
  "Name of the Git executable (excluding any arguments)."
  :type 'string
  :group 'git-gutter)

(defcustom git-gutter:svn-program "svn"
  "Name of the Subversion executable (excluding any arguments)."
  :type 'string
  :group 'git-gutter)

(defcustom git-gutter:hg-program "hg"
  "Name of the Mercurial executable (excluding any arguments)."
  :type 'string
  :group 'git-gutter)

(defcustom git-gutter:bzr-program "bzr"
  "Name of the Bazaar executable (excluding any arguments)."
  :type 'string
  :group 'git-gutter)
...
(defun git-gutter:in-git-repository-p ()
  (when (executable-find git-gutter:git-program)
    (with-temp-buffer
      (when-let ((exec-result (git-gutter:execute-command
                               git-gutter:git-program t "rev-parse" "--is-inside-work-tree")))
...
(defun git-gutter:vcs-check-function (vcs)
  (cl-case vcs
    ...
    (svn (git-gutter:in-repository-common-p git-gutter:svn-program '("info") ".svn"))
    (hg (git-gutter:in-repository-common-p git-gutter:hg-program '("root") ".hg"))
    (bzr (git-gutter:in-repository-common-p git-gutter:bzr-program '("root") ".bzr"))))
...
(defun git-gutter:start-git-diff-process (file proc-buf)
  (let ((arg (git-gutter:git-diff-arguments file)))
    (apply #'start-file-process "git-gutter" proc-buf
           git-gutter:git-program "--no-pager" "-c" "diff.autorefreshindex=0"
...
(defsubst git-gutter:start-svn-diff-process (file proc-buf)
  (let ((args (git-gutter:svn-diff-arguments file)))
    (apply #'start-file-process "git-gutter" proc-buf git-gutter:svn-program "diff" "--diff-cmd"
           "diff" "-x" "-U0" args)))
...
(defsubst git-gutter:start-hg-diff-process (file proc-buf)
  (let ((args (git-gutter:hg-diff-arguments file))
    ...
    (apply #'start-file-process "git-gutter" proc-buf git-gutter:hg-program "diff" "-U0" args)))
...
(defsubst git-gutter:start-bzr-diff-process (file proc-buf)
  (let ((args (git-gutter:bzr-diff-arguments file)))
    (apply #'start-file-process "git-gutter" proc-buf
           git-gutter:bzr-program "diff" "--context=0" args)))
  ...
  (git-gutter:awhen (git-gutter:base-file)
    (with-temp-buffer
      (when (zerop (git-gutter:execute-command
                    git-gutter:git-program t "--no-pager" "-c" "diff.autorefreshindex=0"
    ...
    (when header
      ...
      (let ((dir-option (git-gutter:apply-directory-option))
        ...
        (unless (zerop (apply #'git-gutter:execute-command
                              git-gutter:git-program nil "apply" "--unidiff-zero"
                              options))
...
(defun git-gutter:revision-valid-p (revision)
  (zerop (cl-case git-gutter:vcs-type
           (git (git-gutter:execute-command git-gutter:git-program nil
                                            "rev-parse" "--quiet" "--verify"
                                            revision))
           (svn (git-gutter:execute-command git-gutter:svn-program nil "info" "-r" revision
                                            (file-relative-name (buffer-file-name))))
           (hg (git-gutter:execute-command git-gutter:hg-program nil "id" "-r" revision))
           (bzr (git-gutter:execute-command git-gutter:bzr-program nil
                                            "revno" "-r" revision)))))
...
(defun git-gutter:original-file-content (file vcs)
  (with-temp-buffer
    (cl-case vcs
      (git
       (when (zerop (process-file git-gutter:git-program nil t nil "show" (concat ":" file)))
...
(defun git-gutter:vcs-root (vcs)
  (with-temp-buffer
    (cl-case vcs
      (git
       (when (zerop (process-file git-gutter:git-program
                                  nil t nil "rev-parse" "--show-toplevel"))
         (goto-char (point-min))
         (file-name-as-directory
          (buffer-substring-no-properties (point) (line-end-position)))))
      (svn
       (when (zerop (process-file git-gutter:svn-program nil t nil "info"))
         (goto-char (point-min))
