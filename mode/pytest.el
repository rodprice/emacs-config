...
;; 12 Nov 2018 Changes made to allow use under Windows.  The author,
;; bless his heart, added support for projectile.  In the process, he
;; removed support for Windows, inadvertantly or not.  
...
(defun pytest-run-command (use-comint command)
  (compilation-start
   command
   use-comint
...
;;; Utility functions
(defun root-directory-p (dirname)
  (let ((up-dir (file-name-directory (directory-file-name dirname))))
    (string-equal dirname up-dir)))
